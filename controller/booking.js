...
async function booking(req,res){
    ...
    try{
        ...
        // const idProof = req.body.file
        const idProof = req.file
        console.log(idProof,"oooooooooooooooooooooooooooooooooooooooooooooooooooo");
        const upId =idProof.filename
        console.log(upId,"mmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmm");
        // path = path.replace("/", "//")
       const userBooking= bookingModel.booking(name,date,time,physician,upId)
        
       if(userBooking ==false){
        return res.json({ success:false , message: 'user is not BOOKed! error occured in db'})
    }
